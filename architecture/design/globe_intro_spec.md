...
1. **우주 등장 + 자전** — 별이 반짝이는 네이비 우주 배경에 3D 지구본(orthographic)이 천천히 자전 (~1.6초)
...
```css
/* 우주 배경 */
.map-stage { background: radial-gradient(1200px 700px at 70% 20%, #0a2552 0%, #061634 45%, #030b1f 100%); }
/* 별 */
@keyframes twinkle { 0%,100%{opacity:.25} 50%{opacity:.9} }
.star { fill:#cde0ff; animation: twinkle 4s ease-in-out infinite; }
/* 육지 */
.country-land { fill:#2a6fb0; stroke:#0a2a55; stroke-width:.4px; transition:fill .25s; }
.country-land.region-active { fill:#7fd0ff; }   /* 포커스/하이라이트 */
.country-land.land-hover    { fill:#58a8e8; cursor:pointer; }
```
SVG defs:
- `#oceanGrad` (radial): `#1e4a8a → #0d2f63 → #04183a`
- `#atmGlow`  (radial): 82% 투명 → 94% `rgba(110,160,255,.28)` → 100% 투명
- `#mkGlow`   (filter): `feGaussianBlur stdDeviation=2.5` + merge (마커 발광)
- 위경도선: `stroke rgba(140,180,255,.16)`, 바다 외곽선 `rgba(120,170,255,.35)`
- 아크선: 베이스라인 `rgba(127,208,255,.55)` 굵게, 후보 `rgba(170,198,255,.3)`, `stroke-dasharray:3,4`
...
- [ ] `#starfield`(뒤) + `#map`(앞) 두 SVG, `.map-stage` 우주 배경
