...
## 7. 디자인 토큰 (색/스타일) — 라이트 테마 (Pantone 660C)

> 기준 색: **Pantone 660C ≈ `#3A75C4`** (밝은 로열블루). 다크모드 아님 — 화이트 배경.
> 핵심 원칙: ① 배경은 화이트~옅은 하늘빛, ② 육지는 660C, ③ **경계선은 흰색이 아니라 육지보다 진한 같은 계열 반투명**(흰선이 튀는 것 방지), ④ 바다는 육지보다 **밝게**(육지가 위로 떠 보이게).

```css
/* 배경: 660C 계열 아주 옅은 하늘빛 → 화이트 */
.map-stage { background: radial-gradient(1200px 760px at 68% 18%, #eef4fc 0%, #f7fafe 48%, #ffffff 100%); }
/* 별 (라이트 배경에 거슬리지 않게 톤다운) */
@keyframes twinkle { 0%,100%{opacity:.15} 50%{opacity:.5} }
.star { fill:#9dbde6; animation: twinkle 4.5s ease-in-out infinite; }
/* 육지: 660C. 경계선은 같은 계열 진한색 반투명 → 자연스럽게 구분(흰선처럼 튀지 않음) */
.country-land { fill:#3A75C4; stroke:rgba(22,70,127,.45); stroke-width:.4px;
                vector-effect:non-scaling-stroke; transition:fill .25s; }
.country-land.region-active { fill:#16467f; stroke:rgba(10,40,80,.55); } /* 포커스/하이라이트 */
.country-land.land-hover    { fill:#2C63B0; cursor:pointer; }
```

> ⚠️ **경계선에 `vector-effect:non-scaling-stroke`** 필수 — 줌인해도 선 두께가 일정하게 유지되어 확대 시 선이 뭉치지 않는다.

SVG defs / 런타임 stroke:
- `#oceanGrad` (radial): `#eaf2fb → #cfe0f5 → #a9c6e8` (육지보다 밝은 옅은 하늘빛)
- `#atmGlow`  (radial): 80% 투명 → 93% `rgba(58,117,196,.22)` → 100% 투명 (660C 푸른빛)
- `#mkGlow`   (filter): `feGaussianBlur stdDeviation=2.5` + merge (마커 발광)
- 바다 외곽선: `rgba(58,117,196,.35)` / 위경도선: `rgba(58,117,196,.18)`
- 아크선: 베이스라인 `rgba(58,117,196,.55)` 굵게, 후보 `rgba(58,117,196,.28)`, `stroke-dasharray:3,4`

### 마커 (라이트 배경 대응)
- halo: `stroke #3A75C4`, opacity 0.45
- core: 베이스라인 `fill #16467f` / 후보 `fill #ffffff` + `stroke #3A75C4`
- 라벨: `fill #16467f` + **흰색 외곽선** `stroke rgba(255,255,255,.95) width 3.5px` (밝은 배경 위 가독성)

> 💡 다크 테마로 되돌리려면: 배경 `#0a2552→#030b1f`, 육지 `#2a6fb0`/경계 `#0a2a55`, 바다 `#1e4a8a→#04183a`, 글로우/선 `rgba(110~140,160~180,255,…)`, 라벨은 밝은 글자+어두운 외곽선으로 반전.
