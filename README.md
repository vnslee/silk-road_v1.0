...
├── app/                        # 애플리케이션 소스 코드
│   ├── backend/
│   │   ├── engine/             # 핵심 로직 엔진 (calculation · generation · rendering)
│   │   └── storage/            # 데이터 저장소 (data 입력 · report 출력)
...
| 영역 | 경로 | 내용 |
|------|------|------|
| 엔진 | [app/backend/engine](app/backend/engine/README.md) | 스코어링·리포트 생성·HTML 렌더링 파이프라인 |
| 저장소 | [app/backend/storage](app/backend/storage/README.md) | 입력 데이터(`data/`)와 생성 결과물(`report/`) |
...
```
research 명세 → AI 리서치 → storage/data (국가·내부 데이터)
   → engine(calculation·generation) → storage/report/.../data (JSON)
   → engine(rendering) → storage/report/.../html (HTML 보고서)
```
